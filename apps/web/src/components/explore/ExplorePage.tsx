...
export function ExplorePage({ children }: { children: ReactNode }) {
  return <div className="explore-page"><main id="main-content">{children}</main></div>;
}
...
export function PageIntro({ label, title, description, children }: {
  ...
  return <header className="explore-intro explore-wrap">
    ...
    <div className="explore-intro-row"><div><h1>{title}</h1><p className="explore-description">{description}</p></div>{children}</div>
  </header>;
}
